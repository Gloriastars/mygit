...
filename = os.path.join(dmax,'model_1.in')
#正演  n：仿真次数（A扫描次数）->B扫描
api(filename, n=1, geometry_only=False)  #geometry_only：仅几何图形
merge_files(r".\GprmaxCode\model_1", removefiles=False)

# 获取回波数据
# A B扫描时out文件名不一样
filename = os.path.join(r".\GprmaxCode\model_1_merged.out")
rxnumber = 1
rxcomponent = 'Ez'
outputdata, dt = get_output_data(filename, rxnumber, rxcomponent)

# 保存回波数据
np.savetxt('model_1.txt',outputdata,delimiter=' ')
